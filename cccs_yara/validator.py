...
from typing_extensions import Annotated, Optional, Set
...
class RuleValidatorModel(BaseModel, extra="allow"):
    ...
    minimum_yara: Optional[Annotated[str, StringConstraints(pattern=r"^\d+\.\d+$")]] = Field(
        default=None,
    ...
    tags: Optional[Annotated[Set[str], BeforeValidator(lambda x: [x] if isinstance(x, str) else x)]] = Field(
        default=None,
    ...
    info: Optional[
        Annotated[
            Set[str],
            BeforeValidator(lambda x: [x.upper()] if isinstance(x, str) else [i.upper() for i in x]),
        ]
    ] = Field(
        default=None,
    ...
    mitre_group: Optional[
        Annotated[
            str,
            BeforeValidator(lambda x: x if isinstance(x, str) else list(set(x))[0]),
            StringConstraints(to_upper=True),
        ]
    ] = Field(
        default=None,
    ...
    report: Optional[Annotated[Set[str], BeforeValidator(lambda x: [x] if isinstance(x, str) else x)]] = Field(
        default=None,
        description="List of reports or references associated with the rule for further reading and context",
    )

    reference: Optional[Annotated[Set[str], BeforeValidator(lambda x: [x] if isinstance(x, str) else x)]] = Field(
        default=None,
    ...
    def after_validation(cls, context):
        ...
        for key, value in list(cls.model_extra.items()):
            if key.startswith("original_"):
                ...
                original_key = key[9:]
                if isinstance(value, int) and value == getattr(cls, original_key, None):
