...
def overwrite_file(path, content):
    if path not in OUTPUT_FILE_LIST:
        with open(path, 'w', encoding='utf-8') as f:
            f.write(content + '\n')
        OUTPUT_FILE_LIST.append(path)
    else:
        # Imports should already be included in the first rebuilt rule
        with open(path, 'a', encoding='utf-8') as f:
            rule_start = max(content.find('rule'), 0)
            f.write('\n' + content[rule_start:] + '\n')
...
def __call_validator(options):
    ...
    for yara_rule_path in list(paths_to_validate):
        ...
        for rule_return in rule_return_list:
            # Prints the output of the validator.
            file_message = '{message:>24}: {y_file:<20} ({y_name!r})'
            if not rule_return.rule_validity:
                # The rule is invalid
                ...
                puts(colored.red(file_message.format(
                    message='🍩🍩Invalid Rule In File',
                    y_file=str(yara_rule_path),
                    y_name=rule_return.rule_name,
                )))
                ...
                puts(colored.yellow(file_message.format(
                    message='Rule Caused Warning In File',
                    y_file=str(yara_rule_path),
                    y_name=rule_return.rule_name,
                )))
                if options.verbose or options.veryverbose:
    ...
    print(dedent('''
    ----------------------------------------------------------------------------
    All Rule files found have been passed through the CCCS YARA Validator:
